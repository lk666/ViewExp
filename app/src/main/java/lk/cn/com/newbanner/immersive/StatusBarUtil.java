...
public class StatusBarUtil {
    ...
    public static float DEFAULT_ALPHA = 0;//Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
    // ? 0.2f : 0.3f;
    ...
    public static void immersive(Activity activity, int color, @FloatRange(from = 0.0, to = 1.0)
            float alpha) {
        immersive(activity.getWindow(), color, alpha);
    }

    public static void immersive(Activity activity, int color, @FloatRange(from = 0.0, to = 1.0)
            float alpha, boolean isLayout) {
        immersive(activity.getWindow(), color, alpha,isLayout);
    }
    ...
    public static void immersive(Window window, int color, @FloatRange(from = 0.0, to = 1.0)
            float alpha) {
        immersive(window, color, alpha, false);
    }
    ...
    public static void darkMode(Activity activity, int color, @FloatRange(from = 0.0, to = 1.0)
            float alpha) {
     ...
     * 设置状态栏darkMode,字体颜色及icon变黑(目前支持MIUI6以上,Flyme4以上,Android M以上)
     */
    public static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float
            alpha) {
        if (isFlyme4Later()) {
...
//        window.setStatusBarColor(Color.TRANSPARENT);
    ...
    public static boolean isFlyme4Later() {
        return Build.FINGERPRINT.contains("Flyme_OS_4")
                ...
                || Pattern.compile("Flyme OS [4|5]", Pattern.CASE_INSENSITIVE).matcher(Build
                .DISPLAY).find();
    }
     ...
     * 创建假的透明栏
     */
    public static void setTranslucentView(ViewGroup container, int color, @FloatRange(from = 0.0,
            to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT >= 19) {
            ...
            if (translucentView == null && mixtureColor != 0) {
                ...
                ViewGroup.LayoutParams lp = new ViewGroup.LayoutParams(
                        ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(container
                        .getContext()));
